...
class AuthService {
  matchUser = async (
    ...
    const image =
      user.admin?.photo ?? user.customer?.logo ?? user.employee?.photo;
    return {
      id: user.id,
      email: user.email,
      name,
  ...
  async createResetPasswordRequest(
    ...
    await mailService.send(
      "reset-password.ejs",
      {
        ...
        expiresAt: `${[
          resetTokenExpiresAt.getDate().toString().padStart(2, "0"),
          (resetTokenExpiresAt.getMonth() + 1).toString().padStart(2, "0"),
          resetTokenExpiresAt.getFullYear(),
        ].join("/")} à ${[
          resetTokenExpiresAt.getHours().toString().padStart(2, "0"),
          resetTokenExpiresAt.getMinutes().toString().padStart(2, "0"),
        ].join(":")}`,
        token: resetToken,
      },
      {
      ...
      if (user.resetTokenExpiresAt && user.resetTokenExpiresAt > new Date()) {
        ...
        await prisma.user.update({
          ...
          data: {
            ...
            resetTokenExpiresAt: null, // Clear the expiration date
            password: await hash(password, 12), // Hash the new password
          },
        });
        return "passwordUpdatedSuccessfully";
      } else {
        return "resetTokenExpired"; // Token has expired
      }
    } else {
      return "passwordResetError"; // User not found
    }
  }
}
